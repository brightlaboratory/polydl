...
#ifndef USELIBXSMM_GEMM 
#define USELIBXSMM_GEMM 0
#endif // !USELIBXSMM_GEMM
...
double libxsmm_gemm_baseline(float A[M1][K1], float B[K1][N1], float C[M1][N1],
	int iters) {

	printf("libxsmm_gemm_baseline\n");
	int it, jt, kt, i, j, k;
	int lda = K1;
	int ldb = N1;
	int ldc = N1;
...
int main() {
	...
	// matmul_gemm(A, B, C);
	if (USELIBXSMM_GEMM == 0) {
		tiled_matmul_gemm_baseline(A, B, C, 1);
	}
	else {
		libxsmm_gemm_baseline(A, B, C, 1);
	}
	...
	if (USELIBXSMM_GEMM == 0) {
		l_total = tiled_matmul_gemm_baseline(A, B, C, NUM_ITERS);
	}
	else {
		l_total = libxsmm_gemm_baseline(A, B, C, NUM_ITERS);
	}
